...
public class Vector {
	...
	public void changeDim(int new_dim) throws LinAlgException{
		...
		double[] new_vec;
		new_vec = new double[new_dim];
		for (int i = 0 ; i< new_dim; i++) { // fill out the vector with 0.0
			new_vec[i] = 0.0;
		}
		int leng = _nDim;
			for (int j = 0; j<leng; j++) { // fill out the new vector with the elements in the original vector
				if (j== new_dim) //check if the new vector is fully filled.
					break;
				new_vec[j] = _adVal[j];		

				
			}
			_nDim = new_dim;	//reallocating array storage
			_adVal= new_vec;
	}
	...
	public Vector scalarAdd(double d) {
		...
		Vector vv = new Vector(this) ;
		vv.scalarAddInPlace(d);
		return vv;
	}
	...
	public Vector elementwiseAdd(Vector v) throws LinAlgException {
		// TODO (this should not return null!)
		if (this._nDim != v._nDim) {
			throw new LinAlgException("Cannot elementWiseAdd vectors of different dimensions "+ _nDim + " and "+v._nDim);		
		}
			Vector vv = new Vector (this);
			vv.elementwiseAddInPlace(v);
			return vv;
	}
	...
	public void elementwiseMultInPlace(Vector v) throws LinAlgException {
		// TODO
		if (this._nDim != v._nDim) {
			throw new LinAlgException("Cannot elementWiseMult vectors of different dimensions "+this._nDim +" and " +v._nDim);
		}
		int i = _nDim ;
		for (int index = 0; index <i; index++)
			_adVal[index] *= v._adVal[index];
	...
	public static double InnerProd(Vector v1, Vector v2) throws LinAlgException {
		...
		if (v1._nDim != v2._nDim) {
			throw new LinAlgException("Cannot innerProd vectors of different dimensions "+ v1._nDim +" and " + v2._nDim);// check if two vectors' dimensions equal
		}
		int i = v1._nDim ;
		int sum = 0;
		for (int index = 0; index <i; index++)
			sum += v1._adVal[index] * v2._adVal[index];// Values from corresponding indices multiply
		return sum;
	}
}
